...
fn load(name: &str) -> Structure {
    read_structure(&Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/data").join(name), None, false).unwrap()
}
...
fn rotation_about(p: V3, u: V3, deg: f64) -> Transform {
    ...
    Transform { r, t: [p[0] - rp[0], p[1] - rp[1], p[2] - rp[2]] }
}
...
fn transform_residues(s: &mut Structure, from: usize, tr: &Transform) {
    for i in from..s.len() {
        ...
        for a in [&mut b.n, &mut b.ca, &mut b.c, &mut b.o].into_iter().flatten() {
            *a = tr.apply(a);
        }
    }
}
...
fn hinge_motion_is_recovered() {
    ...
    let h = a.len() / 2;
    let axis = {
        let d = [a.ca[h + 1][0] - a.ca[h - 1][0], a.ca[h + 1][1] - a.ca[h - 1][1], a.ca[h + 1][2] - a.ca[h - 1][2]];
    ...
    let r = run(a, b);
    assert!(r.tm_rigid() < 0.8, "rigid TM should drop, got {}", r.tm_rigid());
    assert!(r.tm_flex() > 0.9, "flexible TM {} (rigid {})", r.tm_flex(), r.tm_rigid());
